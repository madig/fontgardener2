...
impl Fontgarden {
    ...
    pub fn load(path: &Path) -> Result<Self, LoadError> {
        ...
        Self::load_glyphs(&mut glyphs, None, path)?;
    ...
    fn load_glyphs(
        ...
        glyphs
            ...
            .filter(|(glyph_name, _)| {
                glyph_set
                    .map(|s| s.contains(glyph_name.as_str()))
                    .unwrap_or(true)
            })
    ...
    fn load_layers(
        ...
        for entry in fs::read_dir(&glyph_dir).map_err(|e| LoadError::Io(glyph_dir.clone(), e))? {
            ...
            let layer: Layer = serde_json::from_reader(layer_file)
                .map_err(|e| LoadError::LoadLayerJson(layer_path.clone(), glyph_name.into(), e))?;
            glyph
                .layers
                .insert(filename_to_name(layer_filename_stem), layer);
        }
        Ok(())
    }
